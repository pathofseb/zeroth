"""Walking task for ZBot - FORWARD WALKING VARIANT
This variant focuses on stable forward walking.
Balanced rewards for forward progress and gait stability.
"""
...
class ZbotWalkingTask(ksim.PPOTask[ZbotWalkingTaskConfig]):
    ...
    def get_rewards(self, physics_model: ksim.PhysicsModel) -> list[ksim.Reward]:
        # FORWARD WALKING VARIANT: Balanced forward locomotion with stability
        return [
            ksim.StayAliveReward(scale=1.0),
            ksim.UprightReward(scale=1.2),  # Slightly increased for stability
            ksim.NaiveForwardReward(scale=6.0, clip_min=None, clip_max=0.2),  # Increased forward reward
