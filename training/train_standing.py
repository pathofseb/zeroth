"""Walking task for ZBot - STANDING VARIANT
This variant focuses on standing still with minimal movement.
Emphasizes upright posture, stability, and penalizes movement.
"""
...
class ZbotWalkingTask(ksim.PPOTask[ZbotWalkingTaskConfig]):
    ...
    def get_rewards(self, physics_model: ksim.PhysicsModel) -> list[ksim.Reward]:
        # STANDING VARIANT: High upright reward, no forward reward, penalize all movement
        return [
            ksim.StayAliveReward(scale=2.0),  # Increased - staying alive is critical
            ksim.UprightReward(scale=3.0),  # Increased - main goal is staying upright
            ksim.NaiveForwardReward(scale=0.0, clip_min=None, clip_max=0.2),  # Disabled - no forward movement
            ksim.NaiveForwardOrientationReward(scale=0.5),  # Keep orientation stable
            ksim.LinearVelocityPenalty(
                index="y",
                in_robot_frame=True,
                norm="l1",
                scale=-3.0,  # Increased - penalize lateral movement
            ),
            SimpleSingleFootContactReward(scale=0.0, stand_still_threshold=None),  # Disabled - not needed for standing
            FeetAirtimeReward(
                scale=0.0,  # Disabled - don't reward lifting feet when standing
                ctrl_dt=self.config.ctrl_dt,
            ...
            FeetOrientationReward.create(
                ...
                target_rp=(0.0, 0.0),
                error_scale=0.25,
                scale=0.8,  # Increased - keep feet level
            ),
            FeetTooClosePenalty(
                feet_pos_obs_key="feet_position_observation",
                threshold_m=0.12,
                scale=-0.1,  # Increased penalty
            ),
            StraightLegPenalty.create_penalty(physics_model, scale=-0.3, scale_by_curriculum=True),  # Reduced - allow some flex
            AnkleKneePenalty.create_penalty(physics_model, scale=-0.015, scale_by_curriculum=True),  # Reduced
            ksim.ActionVelocityPenalty(scale=-0.05,  scale_by_curriculum=True),  # Enabled - penalize rapid actions
            ksim.JointVelocityPenalty (scale=-0.05,  scale_by_curriculum=True),  # Enabled - penalize joint movement
            ContactForcePenalty(
                 scale=-0.02,
                 sensor_names=("sensor_observation_left_foot_force", "sensor_observation_right_foot_force"),
            ),
            ArmPosePenalty.create_penalty(physics_model, scale=-0.8, scale_by_curriculum=True),  # Increased - keep arms still
            ksim.ReachabilityPenalty(
                delta_max_j=tuple(float(x) for x in self.delta_max_j),
                scale=-0.5,  # Reduced - allow some movement for balance
                squared=False,
