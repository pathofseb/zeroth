"""Walking task for ZBot - AGILE WALKING VARIANT
This variant emphasizes quick, dynamic movements.
Reduced action/joint velocity penalties for agility.
"""
...
class ZbotWalkingTask(ksim.PPOTask[ZbotWalkingTaskConfig]):
    ...
    def get_rewards(self, physics_model: ksim.PhysicsModel) -> list[ksim.Reward]:
        return [
            ...
            StraightLegPenalty.create_penalty(physics_model, scale=-0.3, scale_by_curriculum=True),  # Reduced - allow flexibility
            AnkleKneePenalty.create_penalty(physics_model, scale=-0.015, scale_by_curriculum=True),  # Reduced
            # ACTION/JOINT penalties greatly reduced for agility
            # ksim.ActionVelocityPenalty(scale=-0.005,  scale_by_curriculum=True),  # Very low
            # ksim.JointVelocityPenalty (scale=-0.005,  scale_by_curriculum=True),  # Very low
            # ksim.JointAccelerationPenalty(scale=-0.005, scale_by_curriculum=True),  # Very low
