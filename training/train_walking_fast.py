"""Walking task for ZBot - FAST WALKING VARIANT
This variant emphasizes maximum forward speed.
Higher forward reward, reduced stability penalties to allow faster gaits.
"""
...
class ZbotWalkingTask(ksim.PPOTask[ZbotWalkingTaskConfig]):
    ...
    def get_rewards(self, physics_model: ksim.PhysicsModel) -> list[ksim.Reward]:
        # FAST WALKING VARIANT: Maximize forward speed
        return [
            ksim.StayAliveReward(scale=1.0),
            ksim.UprightReward(scale=0.8),  # Reduced - allow some tilt for speed
            ksim.NaiveForwardReward(scale=10.0, clip_min=None, clip_max=0.3),  # Doubled for speed, higher clip
            ksim.NaiveForwardOrientationReward(scale=0.2),  # Reduced - less strict orientation
            ksim.LinearVelocityPenalty(
                index="y",
                in_robot_frame=True,
                norm="l1",
                scale=-0.5,  # Reduced - allow more lateral sway for speed
            ),
            SimpleSingleFootContactReward(scale=0.4, stand_still_threshold=None),  # Increased - faster gait
            FeetAirtimeReward(
                scale=0.2,  # Increased - reward longer strides
                ctrl_dt=self.config.ctrl_dt,
                touchdown_penalty=0.05,  # Reduced - allow faster stepping
            ...
            FeetOrientationReward.create(
                ...
                target_rp=(0.0, 0.0),
                error_scale=0.25,
                scale=0.2,  # Reduced - less strict on foot orientation
            ),
            FeetTooClosePenalty(
                feet_pos_obs_key="feet_position_observation",
                threshold_m=0.12,
                scale=-0.02,  # Reduced - allow closer feet for faster gait
            ),
            StraightLegPenalty.create_penalty(physics_model, scale=-0.3, scale_by_curriculum=True),  # Reduced
            AnkleKneePenalty.create_penalty(physics_model, scale=-0.015, scale_by_curriculum=True),  # Reduced
            ContactForcePenalty(
                 scale=-0.05,  # Increased - penalize hard impacts at speed
                 sensor_names=("sensor_observation_left_foot_force", "sensor_observation_right_foot_force"),
            ),
            ArmPosePenalty.create_penalty(physics_model, scale=-0.3, scale_by_curriculum=True),  # Reduced - allow arm swing
            ksim.ReachabilityPenalty(
                delta_max_j=tuple(float(x) for x in self.delta_max_j),
                scale=-0.5,  # Reduced - allow aggressive movements
                squared=False,
