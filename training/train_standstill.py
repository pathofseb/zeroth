"""Walking task for ZBot - STANDSTILL VARIANT.

This variant focuses on learning to stand still and balance.
Simplified rewards: only StayAlive and Upright.
"""
...
class ZbotWalkingTask(ksim.PPOTask[ZbotWalkingTaskConfig]):
    ...
    def get_rewards(self, physics_model: ksim.PhysicsModel) -> list[ksim.Reward]:
        """Get rewards - STANDSTILL VARIANT: simplified to only reward staying alive and staying upright.

        This training configuration rewards the robot for:
        1. Staying alive (not falling or terminating) - scale=4.0 (higher than default)
        2. Staying upright (maintaining vertical orientation) - scale=0.5 (lower, allowing some tilt)
        
        All other rewards are disabled to focus on pure balance and stability.
        """
        return [
            ksim.StayAliveReward(scale=4.0),  # STANDSTILL TUNING: High reward for staying alive
            ksim.UprightReward(scale=0.5),     # STANDSTILL TUNING: Moderate reward for staying upright
        ]

    def get_terminations(self, physics_model: ksim.PhysicsModel) -> list[ksim.Termination]:
        """STANDSTILL VARIANT: Adjusted termination conditions for standing still training.
        
        - BadZTermination: base height < 0.15m or > 4.5m (more lenient than default)
        - NotUprightTermination: tilt > 60 degrees  
        - No episode length termination (let it learn longer)
        """
        return [
            ksim.BadZTermination(unhealthy_z_lower=0.15, unhealthy_z_upper=4.5),  # STANDSTILL: More lenient height
            ksim.NotUprightTermination(max_radians=math.radians(60)),
            # No EpisodeLengthTermination - let episodes run indefinitely for standing practice
        ]
